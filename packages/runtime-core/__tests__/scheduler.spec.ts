...
describe('scheduler', () => {
  ...
  it('nextTick call  time', async () => {
    ...
    nextTick(job2) // 通过 resolvePromise 存入微任务队列
    queueJob(job) // 通过 resolvePromise 存入微任务队列
    nextTick(job3) // 通过 currentPromise 存储微任务队列

    expect(calls.length).toBe(0)
    await nextTick()
    expect(calls).toMatchObject(['job2', 'job1', 'job3'])
  })

  it('promise', async () => {
    const dummyThen = Promise.resolve()
  ...
  test('job and pre job sort', async () => {
    debugger
    const calls: string[] = []
    const job1: SchedulerJob = () => {
      calls.push('job1')
    ...
    const preCb1: SchedulerJob = () => {
      calls.push('cb1')
    }
    preCb1.pre = true

    const preCb2: SchedulerJob = () => {
      calls.push('cb2')
    }
    preCb2.pre = true
    preCb2.id = 10
    ...
    expect(calls).toMatchObject(['cb2', 'job2', 'job1', 'cb1'])
  })
})
