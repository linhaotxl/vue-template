...
function computed<T = any>(getter: () => T) {
  let dirty = false
  let value: T
  ...
  const obj = {
    get value() {
      if (!dirty) {
        console.log('重新计算')
        value = effectFn() as unknown as T
        dirty = true
      }
      track(obj, 'value')
      return value
...
// const name = computed(() => ovserver.name)
...
// debugger
/**
 * 一个计算属性内部拥有自己的 effect，并且它是懒执行的
 * 也就是说，只有当真正获取值的时候才会追踪依赖，并且收集的依赖只会存储在自己的 effect 中
 * 如果存在 effect 嵌套，computed 外层的 effect 是收集不到依赖的
 *
 * 所以可以在访问 computed 的时候手动追踪 value 属性，这样就能 value 就能追踪到外部的 effect
 * 而在修改值时，由于会触发 computed 的 schecule，可以在此时再次触发追踪 value 的 effect
 */
// debugger
const sumRes = computed(() => ovserver.name)
